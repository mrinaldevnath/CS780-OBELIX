...
CONFIG = {
    ...
    "max_episodes": 500,
    "replace_trace": False,
    "policy_file": "weights/sarsa_lambda_v1.pkl",
}
...
def sarsa_lambda(env, config):
    Q = defaultdict(lambda: np.zeros(len(ACTIONS)))
    rng = np.random.default_rng(config["seed"])

    trace_threshold = 1e-4

    for e in tqdm(range(config["max_episodes"]), desc="SARSA(λ) Training", unit="ep"):
        ...
        while not done:
            ...
            td_target = r
            if not done:
                td_target += config["gamma"] * Q[s_prime][a_prime_idx]

            td_error = td_target - Q[s][a_idx]

            E[s][a_idx] += 1
            if config.get("replace_trace", True):
                E[s][a_idx] = min(E[s][a_idx], 1.0)
