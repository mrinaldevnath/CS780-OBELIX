...
ALGO_NAMES = ["NFQ", "DQN", "DDQN", "D3QN_PER", "D3QN"]

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[
        logging.FileHandler("batch_evaluation_discrete.log", mode="a"),
        logging.StreamHandler(),
    ],
)


def create_value_network(in_dim, out_dim):
    """Standard Q-network used by NFQ, DQN, and DDQN."""
...
class DuelingNetwork(nn.Module):
    """Dueling architecture used by D3QN and D3QN_PER."""
...
def get_network(algo_name, in_dim, out_dim):
    """Return the correct network architecture for a given algorithm."""
    if algo_name in ["NFQ", "DQN", "DDQN"]:
        return create_value_network(in_dim, out_dim)
    elif algo_name in ["D3QN", "D3QN_PER"]:
        return DuelingNetwork(in_dim, out_dim)
    else:
        raise ValueError(f"Unknown algorithm: {algo_name}")


def infer_algo_name(filename):
    """Extract algorithm name from a filename like 'D3QN_PER_v1.pth'."""
...
def run_episodes(net, env_config, runs, base_seed):
    """Run greedy evaluation episodes and return list of total rewards."""
...
def eval_worker(task_spec):
    """Evaluates a single trained deep RL model across both env conditions."""
    ...
    try:
        net = get_network(algo_name, in_dim, out_dim)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Batch evaluate deep RL models concurrently.")
    parser.add_argument(
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for count, future in enumerate(concurrent.futures.as_completed(futures), 1):
            ...
            if "Error" in res:
                ...
                results.append(res)
                logging.info(
                    f"[{count}/{len(tasks)}] Evaluated: {res['Algorithm']} (Comb Mean: {res['Comb Mean']:.2f})"
                )
    ...
    output_csv = "eval_results_discrete.csv"
