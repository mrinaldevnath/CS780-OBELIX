...
CONFIG = {
    ...
    "batchSize": 256,
    "optimizerLR": 1e-3,
    "policy_file": "weightsPth/DQN_v1.pth",
}
...
class DQN:
    def __init__(
        ...
        self.onlineNet = createValueNetwork(inDim, outDim)
        self.targetNet = createValueNetwork(inDim, outDim)
    ...
    def trainAgent(self):
        pbar = tqdm(range(self.MAX_TRAIN_EPISODES), desc="Training DQN", unit="ep")
        for episode in pbar:
    ...
    def trainNetwork(self, experiences):
        ...
        with torch.no_grad():
            max_next_q = self.targetNet(next_states_t).max(1, keepdim=True)[0]
...
if __name__ == "__main__":
    ...
    print("Starting Deep Q-Learning...")
    final_reward = agent.runDQN()
